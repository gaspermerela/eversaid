...
/**
 * Pre-loaded demo entry data from backend
 *
 * This represents a demo transcription that is pre-computed and served from
 * static files on the backend. Demo entries are NOT stored in Core API - they
 * exist only as static JSON files mounted in the backend's data/demo directory.
 *
 * The rawSegments and cleanedSegments use the same types as Core API responses.
 * This allows us to reuse transformApiSegments() and get word-level timing
 * for playback highlighting.
 *
 * To create demo data:
 * 1. Transcribe audio with Core API
 * 2. Copy raw_transcription_data.segments to rawSegments
 * 3. Copy cleanup_data.segments to cleanedSegments
 *
 * Why demo entries are different:
 * - They load instantly (no transcription wait)
 * - Edits are stored in localStorage (not Core API) - see lib/demo-storage.ts
 * - They're identified by locale ("en" or "sl"), not UUID
 * - They cannot be deleted by users
 */
